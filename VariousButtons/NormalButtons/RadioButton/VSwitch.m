...
#pragma mark VSwitchFaceLayer

@interface VSwitchFaceLayer : CALayer
...
@implementation VSwitchFaceLayer

#pragma mark  Setter Get Method
 ...
 * layer
 */
-(instancetype)initWithLayer:(VSwitchFaceLayer *)layer {
...
@interface VSwitch () <CAAnimationDelegate>{
    CGFloat _sideOfHead;
    ...
    SwitchDidSelectedBlock _SwitchDidSelectedBlock;
}
...
-(void)initLBWSwitch {
    ...
    self.faceLayer = [VSwitchFaceLayer layer];
...
-(void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
    if (flag) {
        ...
        if (anim == [_faceLayer animationForKey:kFaceLayerEndMoveAnimationKey]) {
            ...
            isAnimating = NO;
            if (_SwitchDidSelectedBlock) {
                _SwitchDidSelectedBlock(_isOn);
            }
            return;
        }
