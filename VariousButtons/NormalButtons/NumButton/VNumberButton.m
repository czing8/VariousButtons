...
@interface VNumberButton () <UITextFieldDelegate>
...
@implementation VNumberButton

#pragma mark - 初始化
+ (instancetype)numberButtonWithFrame:(CGRect)frame {
    return [[VNumberButton alloc] initWithFrame:frame];
}
...
- (void)textFieldDidEndEditing:(UITextField *)textField {
    ...
    _numberHandler ? _numberHandler(_textField.text) : nil;
}
...
- (void)increase {
    ...
    if (number <= _maxValue) {
        ...
        if (_isHideDecrease && number == _minValue) {
        ...
        if (_enableShake) { [self shakeAnimationMethod]; }
    }
}
...
@implementation NSString (VNumberButton)
